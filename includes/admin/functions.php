...
function gahb_approve_link_in_bookings($actions, $post)
{
    if ($post->post_type=='gahb_booking')
    {
		...
		{
			$actions['approve_booking'] = '<a href="post.php?post='.$post->ID.'&action=unapprove_booking" title="" rel="permalink">Unapprove Booking</a>';
		}
    }
    return $actions;
}
...
function unapprove_booking_post_type_func () 
{
	if(!(isset( $_GET['post']) || (isset( $_REQUEST['action']) && 'archive' == $_REQUEST['action'] )))
	{
    	wp_die( 'No post supplied!');
    ...
    if($id)
	{
		...
		update_post_meta( $id, "booking_status", "0");
		global $wpdb;
		$prefix = $wpdb->prefix;;
		$table_name = $prefix."gahb_bookings";
		$wpdb->query("update $table_name set status=0 where booking_id = $id");
...
add_action( 'admin_action_unapprove_booking', 'unapprove_booking_post_type_func');

function get_price_string( $price ){
	$ret_price = $price;
	$currency = get_option('gahb_currency', '$');
	$position = get_option('gahb_currency_position', 'pos_before');
	switch($position){
		case 'pos_before':
			$ret_price = $currency.$price;
		break;
		
		case 'pos_after':
			$ret_price = $price.$currency;
		break;
		
		case 'pos_before_space':
			$ret_price = $currency.' '.$price;
		break;
		
		case 'pos_after_space':
			$ret_price = $price.' '.$currency;
		break;
